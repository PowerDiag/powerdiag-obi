...
INTERFACE_VOLTAGE_CMD   = [0x01, 0x00, 0x02, 0x02]
...
class Interface(tk.Frame):
    def __init__(self, parent, obi_instance):
        ...
        # None = not probed yet, False = this interface cannot measure the voltage
        self.voltage_supported = None
        self.create_widgets()
    ...
    def get_pack_voltage(self):
        """Pack voltage measured by the interface on the battery terminals, in volts.

        Requires an interface board with the voltage divider fitted and firmware
        0.3.0 or newer. Returns None if the interface cannot supply a reading;
        stock OBI hardware simply does not answer this command, and is then not
        asked again for the rest of the connection.
        """
        if self.voltage_supported is False:
            return None

        try:
            response = self.request(INTERFACE_VOLTAGE_CMD, max_attempts=1)
            voltage = int.from_bytes(response[2:4], byteorder='little') / 1000
            self.voltage_supported = True
            return voltage
        except Exception as e:
            self.voltage_supported = False
            self.obi_instance.update_debug(
                f"Interface does not report a pack voltage, skipping it from now on: {e}"
            )
            return None
