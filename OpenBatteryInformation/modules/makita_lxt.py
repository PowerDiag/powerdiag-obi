...
class ModuleApplication(tk.Frame):
    ...
    def get_f0513_model(self):
        try:
            ...
            raise e
    def update_terminal_voltage(self):
        """Show the voltage the interface measures on the battery terminals.

        Taken once per read, not polled. Silently skipped for interfaces that
        cannot measure it.
        """
        if not hasattr(self.interface, "get_pack_voltage"):
            return

        voltage = self.interface.get_pack_voltage()
        if voltage is None:
            return

        self.insert_battery_data({"Terminal Voltage (measured)": f"{voltage:.2f} V"})
